...
<script type="text/javascript">
    ...
    function getarea(district_id) {
        ...
        $("#area").html("loading ... ");
        $.ajax({
            url: "<?php echo base_url();?>sales/GetArea",
            type: "post",
            data: "id=" + district_id+"&gov="+$("#governorate_id").val()+"&sales_man="+$("#sales_man").val(),
            success: function (result) {
                $("#area").html(result);
            }
        });
    }
    function getarea1(district_id) {
        var chkArray = [];
        ...
        $.ajax({
            url: "<?php echo base_url();?>sales/GetArea",
            type: "post",
            data: "id=" + district_id+"&sales_man="+$("#sales_man").val()+"&category="+selected,
            success: function (result) {
                $("#area").html(result);
            }
        });
    }
    function getSalesmanCompanies(sales_man) {
        $("#company_nbr").html('Loading.....');
        $.ajax({
            url: "<?php echo base_url();?>delivery/GetSalesmanCompanies",
            type: "post",
            data: "id=" + sales_man+"&category="+$("#category").val(),
            success: function (result) {
            ...
            <div class="span12">
                ...
                <div class="block-fluid">
                    <div class="row-form clearfix">
                       ...
                       <?php /* <div class="span2">Category </div>
                        <div class="span1"><?='Company'.form_checkbox(array('name'=>'category[]','value'=>'company','class'=>'chk'))?></div>
                        <div class="span1"><?='Insurance'.form_checkbox(array('name'=>'category[]','value'=>'insurance','class'=>'chk'))?></div>
                        <div class="span1"><?='Importer'.form_checkbox(array('name'=>'category[]','value'=>'importer','class'=>'chk'))?></div>
                        <div class="span1"><?='Transport'.form_checkbox(array('name'=>'category[]','value'=>'transport','class'=>'chk'))?></div>
                   */?>
                    </div>
                    <div class="row-form clearfix">
                        <div class="span2">Start Date</div>
                        <div class="span4"><?php echo form_input(array('name'=>'start_date','value'=>@$query['start_date'],'class'=>'datep')); ?><div class="required"><?=form_error('start_date'); ?></div></div>
                        <div class="span2">Due Date</div>
                        <div class="span4"><?php echo form_input(array('name'=>'due_date','value'=>@$query['due_date'],'class'=>'datep')); ?><div class="required"><?=form_error('due_date'); ?></div></div>
                    </div>
                    <div class="row-form clearfix">
                        <?php
                        ...
                        <div class="span3"><?php echo form_dropdown('sales_man_id',$array_sales_men,@$query['sales_man_id'],' class="select2" id="sales_man" style="width:100% !important" onchange="getSalesmanCompanies(this.value)"'); ?><?=form_error('sales_man_id'); ?></div>
                        ...
                        <div class="span2"><?php echo form_radio(array('name'=>'type','class'=>'category','value'=>'area')).' &nbsp;Area'; ?><div class="required" id="all-area"><?=form_error('type'); ?></div></div>
                        </div>
                        </div>
                    <div class="row-form clearfix" id="company-area">
                        <div class="span2">Company</div>
                        <div class="span10"><?=form_multiselect('company_ids[]',@$array_companies,@$query['company_id'],'id="company_id" style="width:100% !important"'); ?>
                            <div class="required"><?php echo form_error('company_id'); ?></div>
                        </div>
