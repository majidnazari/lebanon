...
<script type="text/javascript" src="<?=base_url()?>ptable/scripts/jquery-1.8.3.min.js"></script>
<script type="text/javascript" src="<?=base_url()?>table/scripts/move.js"></script>

<?php 
$array_name_ar=array('id'=>'name_ar','name'=>'name_ar','value'=>$name_ar,'style'=>'direction:rtl !important; text-align:right !important;');
$array_name_en=array('id'=>'name_en','name'=>'name_en','value'=>$name_en);
?>
<script language="javascript">
function editdirector(id,name_ar,name_en)
	{
...
function save(id)
	{
		...
		$.ajax({
			url: "<?php echo base_url();?>banks/editdirector",
			type: "post",
			data: "id="+id+"&namear="+namear+"&nameen="+nameen,
			success: function(result){
				$("#"+id+"ar").html(namear);
				$("#"+id+"en").html(nameen);
				$("#"+id+"btn").html('<a href="javascript:void(0)" onclick="editdirector('+id+',\''+namear+'\',\''+nameen+'\')"><i class="isb-edit"></i></a>');
				$("#"+id+"-tr").children('td, th').css('background-color','yellow');
   				 $("#"+id+"-tr").css('background', 'yellow'); 
...
function standing()
        {
			checkboxes = document.getElementsByName('checkall');
			checkboxes.checked =true;
            document.getElementById("form_id").action = "<?=base_url().'banks/standing'?>";
			document.getElementById("form_id").submit();
        }
        ...
        <div class="row-fluid">
            <div class="span12">
                ...
                <?php echo form_open('',array('id'=>'form_id','name'=>'form1'));
					  echo form_hidden('bank_id',$id);
					  echo form_hidden('p','directors');
					?>   
                ...
                <div class="block-fluid table-sorting clearfix">
                    <table cellpadding="0" cellspacing="0" width="100%" class="table horizontal">
						...
						foreach($directors as $row){
							...
							if($row->update_time >= date('Y-m-d 00:00:00') and $row->update_time <= date('Y-m-d 23:59:59'))
							{
								$class='hight';	
							}
							else{
                            ...
                            <tr id="<?=$row->id?>-tr" class="<?=$class?>">
                                ...
                                <div id="<?=$row->id?>btn">
                                	<a href="javascript:void(0)" onclick="editdirector(<?=$row->id?>,'<?=$row->name_ar?>','<?=$row->name_en?>')"><i class="isb-edit"></i></a>
                                 </div>   
								<?=_show_delete('banks/delete/id/'.$row->id.'/bank/'.$row->bank_id.'/st/director',$p_directors);?>                                        
                                </td>                                    
                            </tr>
                        ...
                        <tfoot>
                        	<tr>
                                <td colspan="7">Total : <?=count($directors)?></td></tr>
                        </tfoot>
                    </table>
