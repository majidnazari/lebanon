                ...
                <div class="block-fluid table-sorting clearfix">
                    <table cellpadding="0" cellspacing="0" width="100%" class="table">
                        ...
                        <tr>
                            ...
                            <th style="text-align:center" width="10%" rowspan="2">Action</th>
                        </tr>
                        <tr>
                        ...
                        if(count($query)>0){
                            foreach($query as $row){
                                ...
                                <tr>
                                    ...
                                    <td style="text-align:center"><?=$row->DeliveryStartDate?></td>
                                    <td style="text-align:center"><?=$row->DeliveryEndDate?></td>
                                    <td style="text-align:center"><?=anchor('delivery/items/'.$row->id,$row->TotalPending+$row->TotalDone,array('class'=>'btn btn-mini'))?></td>
                                    <td style="text-align:center"><?=anchor('delivery/listview/'.$row->id,'<span class="isb-print"></span>',array('target'=>'_blank'))?></td>
                                    <td style="text-align:center"><?=anchor('delivery/detailsview/'.$row->id,'<span class="isb-print"></span>',array('target'=>'_blank'))?></td>
                                    <td style="text-align:center"><?=anchor('delivery/pages/'.$row->id,'<span class="isb-print"></span>',array('target'=>'_blank'))?></td>
                                    <td style="text-align:center"><?=anchor('delivery/items/'.$row->id.'/pending',$row->TotalPending,array('class'=>'btn btn-mini btn-warning'))?></td>
                                    <td style="text-align:center"><?=anchor('delivery/listview/'.$row->id.'/pending','<span class="isb-print"></span>',array('target'=>'_blank'))?></td>
                                    <td style="text-align:center"><?=anchor('delivery/detailsview/'.$row->id.'/pending','<span class="isb-print"></span>',array('target'=>'_blank'))?></td>
                                    <td style="text-align:center"><?=anchor('delivery/pages/'.$row->id.'/pending','<span class="isb-print"></span>',array('target'=>'_blank'))?></td>
                                    <td style="text-align:center"><?=anchor('delivery/items/'.$row->id.'/done',$row->TotalDone,array('class'=>'btn btn-mini btn-success'))?></td>
                                    <td style="text-align:center"><?=anchor('delivery/listview/'.$row->id.'/done','<span class="isb-print"></span>',array('target'=>'_blank'))?></td>
                                    <td style="text-align:center"><?=anchor('delivery/detailsview/'.$row->id.'/done','<span class="isb-print"></span>',array('target'=>'_blank'))?></td>
                                    <td style="text-align:center"><?=anchor('delivery/pages/'.$row->id.'/done','<span class="isb-print"></span>',array('target'=>'_blank'))?></td>
                                    <td style="text-align:center"><?=_show_delete('delivery/delete/'.$row->id.'/delivery',$this->p_delivery_delete)?></td>
                                </tr>

                            <?php } }
                        else{
                            echo '<tr>
                            	<td colspan="17"><center><strong>No Data Found</strong></center></td>
                            </tr>';
                        }
                        ...
                        <tfoot>
                        <tr>
                            <td colspan="17">Total : <?=$total_row?><div class="dataTables_paginate paging_full_numbers"><?=$links?></div></td></tr>
                        </tfoot>
                    </table>
