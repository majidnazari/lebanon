...
function standing()
        {
            ...
            document.getElementById("form_id").action = "<?=base_url().'insurances/standing'?>";
			document.getElementById("form_id").submit();
        }
function editing(id)
        {
			document.getElementById("form_id").action = "<?=base_url().'insurances/edit_executive/'?>"+id;
        ...
        <div class="row-fluid">
            ...
            <?php if($msg!=''){ ?>
                ...
                <?php  echo form_open('',array('id'=>'form_id','name'=>'form1'));
						...
						echo form_hidden('iid',$id);
						echo form_hidden('p','executive');
				?>                                
                <div class="head clearfix">
                    ...
                    <a href="javascript:void(0)" onclick="standing()">
                     ...
                     <?=_show_add_pop('#CreateModal','Add New',$p_executive_add)?> 
                ...
                <table cellpadding="0" cellspacing="0" width="100%" class="table" id="tSortable">
							...
							if( $row->update_time >= $sdate.' 00:00:00' and $row->update_time <= $sdate.' 23:59:59')
							{
                                ...
                                <tr class="<?=$css?>">
                                    ...
                                    <td><?php 
                                         echo _show_delete('insurances/delete/id/'.$row->id.'/iid/'.$id.'/p/executive/',$p_executive_delete);
                                         echo _show_edit_pop('#'.$row->id.'Modal',$p_executive_edit);
                                    ?>                                        
                                    </td>                                    
                            </tr>
                            <?php if($p_executive_edit){ ?>
                            <div id="<?=$row->id?>Modal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
									...
									echo form_open_multipart('',array('id'=>$row->id));
