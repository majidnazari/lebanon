       ...
       <div class="row-fluid">
            <div class="span12">
                 <div class="head clearfix">
                    ...
                    <h1>Search</h1>
                     <ul class="buttons">
                         <li>&nbsp;<?=anchor('tasks/exports?company='.@$company_id.'&list='.@$list.'&sales_man='.@$sales_man.'&year='.@$year.'&from_start='.@$from_start.'&to_start='.@$to_start.'&from_due='.@$from_due.'&to_due='.@$to_due.'&status='.@$status.'&category='.@$category.'&search=Search','<h3>Export To Excel</h3>',array('target'=>'_blank'))?>&nbsp;</li>
                         <li>&nbsp;<?=anchor('tasks/listview/'.$list.'/'.$sales_man.'/'.$status,'<h3>Print List</h3>',array('target'=>'_blank'))?>&nbsp;</li>
                         <li>&nbsp;<?=anchor('tasks/print_all?company='.@$company_id.'&list='.@$list.'&sales_man='.@$sales_man.'&year='.@$year.'&from_start='.@$from_start.'&to_start='.@$to_start.'&from_due='.@$from_due.'&to_due='.@$to_due.'&status='.@$status.'&category='.@$category.'&search=Search','<h3>Print All</h3>',array('target'=>'_blank'))?>&nbsp;</li>
                     </ul>
                 </div>
                  ...
                  <div class="block-fluid">
                    <div class="row-form clearfix">
                        ...
                        foreach($sales as $salesman){
                            ...
                            echo form_dropdown('salesman',$array_sales_men,@$sales_man,' class="select2" style="width:100% !important"'); ?></div>
