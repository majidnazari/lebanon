...
class AG_Auth_model extends CI_Model
{
	...
	public function GetNavigationsByIds($ids=array())
	{
		if(count($ids)>0){
	...
	{
	if($this->db->set('username', $username)->set('password', $password)->set('email', $email)->set('fullname', $fullname)->set('phone',$phone)->set('address',$address)->set('admin_id', $admin_id)->set('status', $status)->set('group_id', $group_id)->set('create_time',date('Y-m-d'))->set('update_time',date('Y-m-d'))->insert($this->user_table))
		{
			return TRUE;
		}
