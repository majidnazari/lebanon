...
class Delivery extends Application
{
    ...
    public function __construct()
    {
        ...
        $this->p_delivery_edit = $this->ag_auth->check_privilege(110, 'edit');
        $this->p_delivery_view = $this->ag_auth->check_privilege(110, 'view');
        $this->p_delivery_delete = $this->ag_auth->check_privilege(110, 'delete');
    ...
    public function GetSalesmanCompanies()
    {
        ...
        switch($category)
        {
            case 'general':
                $x=0;
                $query=$this->Deliverym->GetCompaniesBySalesman($gov='',$district='',$salesmen_all);
...
<script>
    ...
    public function GetArea()
    {
        ...
        $this->data['query']=$areas = $this->Deliverym->GetAreaByDistrictID($gov,$dist_id,$salesmen_all);
        ...
        echo '<div id="TotalArea" style="color: red; font-weight: bold; clear: both"></div>Check All : '.form_checkbox(array('name'=>'area_ids[]','id'=>'checkAll')).'<br>';
        ...
        if (count($areas) > 0) {
            ...
            foreach ($areas as $area) {
                ...
                echo '<td>'.form_checkbox(array('name'=>'area_ids[]','class'=>'check_area','value'=>$area->id,'data-count'=>$area->TotalCompanies)).'</td>';
    ...
    {
     $query = $this->Deliverym->GetGeoDeliveryCompanies('','','','','done');   
     echo '<pre>';
     var_dump($query);
    echo '</pre>';
    ...
    public function delete($id,$type)
    {
        if((int)$id > 0){
            switch($type)
            {
                ...
                case 'delivery':
                    if($this->p_delivery_delete){
                        $query=$this->Deliverym->GetDeliveryTaskById($id);
  ...
  private function InsertGeneralDelivery($sales_id,$sales_man_id,$delivery_id,$status,$area_array=array())
    {
        $query=$this->Deliverym->GetCompaniesBySalesman('','',$sales_man_id,$area_array);
        if(count($query)>0)
        {
            foreach($query as $row)
            {
    ...
    public function index($row = 0)
    {
        ...
        $this->data['subtitle'] = "Delivery Tasks";
   ...
   public function create() {
        ...
        if($this->form_validation->run()) {
            ...
            if(is_array($salesmen_all_array) and count($salesmen_all_array)>0)
            {
            $salesmen_all=implode(',',$salesmen_all_array);
            }
            else{
                $salesmen_all='';
            }
           ...
           /* if(!in_array($salesman,$salesmen_all_array))
            ...
            $data=array(
                ...
                'category' => ($this->input->post('category')!='') ? $this->input->post('category') :'general',
                'status' => $this->input->post('status'),
                'total' => $total,
                'delivery_data'=>$salesmen_all,
                'create_time' => $this->datetime,
                'update_time' => $this->datetime,
                'user_id' => $this->session->userdata('id'),
            );
            ...
            if($delivery_id>0)
            {
                ...
                $area_ids=$this->input->post('area_ids');
                    ...
                    $total=$this->InsertGeneralDelivery($salesman,$salesmen_all,$delivery_id,$this->input->post('status'),$area_ids);
    ...
    public function items($id,$status='')
    {
         if($this->input->get('clear'))
        {
            redirect('delivery/items/'.$id.'/'.$status);
        }
        $get_array=array();
       ...
       $config['base_url'] = '?search=';
        ...
        $query = $this->Deliverym->GetCompanies($id, $status,$row, $limit,$company_id);
 ...
 */
    ...
    public function update_delivery($id)
    {
        ...
        {
            //	$array=$this->Administrator->do_upload('uploads/','docfile');
            ...
            $_array=array(
                    ...
                    'copy_quantity'=>$this->input->post('copy_quantity'),
            ...
            switch($this->input->post('item_type'))
        {
            case 'company':
                $query=$this->Company->SaveDeliveryCompany($_array);
                $this->session->set_userdata(array('done_message' =>'Delivery task(s) updated Successfully'));
                break;
            case 'direct':
    ...
    public function listview($id,$status='')
    {
        ...
        $this->data['delivery']=$this->Deliverym->GetDeliveryTaskById($id);
        $this->data['query'] =$query['rows'];
    ...
    public function salesman_reports($salesman)
    {
        $query=$this->Deliverym->GetDeliveryCompaniesReports($salesman);
        var_dump($query);
    }
    public function salesman_companies()
    {
        ...
        $this->data['status']=$status=$this->input->get('status');
        $query=$this->Deliverym->GetDeliveryCompaniesReports($salesman);
       // $query=$this->Deliverym->GetDeliveryCompanies($salesman,$status,$row,$limit);
        //$query = $this->Deliverym->GetDeliveryTasks($delivery_id,$salesman,$company_id,$status,$row,$limit);
        ...
        $this->breadcrumb->add_crumb('Dashboard', 'dashboard');
        $this->breadcrumb->add_crumb('Delivery');
        $this->breadcrumb->add_crumb('Delivery Reports','delivery');
        $this->data['title'] = $this->data['Ctitle'] . " Delivery Reports";
        $this->data['subtitle'] = "Delivery Reports";
        ...
        $this->data['governorates'] = $this->Address->GetGovernorate('online', 0, 0);
        $this->data['areas'] = $this->Address->GetAreaByDistrict('online', 0);
        $this->data['sales'] = $this->Deliverym->GetSalesMen();
        $this->template->load('_template', 'delivery/salesman_companies', $this->data);

    }
    public function list_details($id,$status='')
    {
        $this->data['id']=$id;
        $this->data['status']=$status;
        $query = $this->Deliverym->GetDeliveryCompanies($id,$status,0,0);
        $this->data['query'] =$query['rows'];
        $this->data['title'] = $this->data['Ctitle'] . " - Companies - الشركات";
        $this->data['subtitle'] = "Companies - الشركات";
        $this->load->view('delivery/list_details', $this->data);
    }
     public function pages($id,$status='')
    {
        $companies=$this->Deliverym->GetCompanies($id,$status,0,0);
